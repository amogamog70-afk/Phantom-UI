...
local Theme = {
    ...
    TabActiveText   = Color3.fromRGB(220, 220, 220),
    LabelText       = Color3.fromRGB(140, 140, 140),
    LabelActiveText = Color3.fromRGB(220, 220, 220),
    ...
    WatermarkBorder = Color3.fromRGB(255, 255, 255),  -- неоновая обводка
    WatermarkText   = Color3.fromRGB(180, 180, 180),
}

-- Список всех UI-элементов для обновления темы
local themeRegistry = {
    buttons   = {},
    labels    = {},
    borders   = {},
...
local function applyTheme()
    for _, b in ipairs(themeRegistry.buttons) do
        if b.Instance and b.Instance.Parent then
            b.Instance.BackgroundColor3 = Theme.ButtonBG
            b.Instance.BorderColor3 = Theme.ButtonBorder
            b.Instance.TextColor3 = Theme.ButtonText
        end
    end
end
...
WatermarkLabel.TextColor3 = Theme.WatermarkText
...
NeonStroke.Color = Theme.WindowBorder
...
StatusLabel.Text = ":: SCRIPT HUB v1.1   |   RSHIFT — TOGGLE"
StatusLabel.TextColor3 = Theme.StatusText
...
local function makeTab(name)
    ...
    tabBtn.MouseButton1Click:Connect(function()
        for _, t in pairs(tabs) do
            ...
            t.btn.TextColor3 = Theme.TabText
    ...
    function section:Section(sName, side)
        ...
        frame.BorderColor3 = Theme.SectionBorder
        ...
        local function updateSize()
            ...
            local leftLayout2 = leftCol:FindFirstChildOfClass("UIListLayout")
            local total = leftLayout2 and leftLayout2.AbsoluteContentSize.Y or 0
        ...
        function sec:Toggle(tName, default, callback)
            ...
            togFrame.Parent = row
            togFrame.BackgroundColor3 = Theme.ToggleBG
            ...
            togInner.BackgroundColor3 = Theme.ToggleOff
            ...
            local function updateVisual()
                if state then
                    TweenService:Create(togInner, TweenInfo.new(0.1), {Position = UDim2.new(0, 15, 0.5, -5), BackgroundColor3 = Theme.Accent}):Play()
                    TweenService:Create(togFrame, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromRGB(20, 20, 20)}):Play()
                    lbl.TextColor3 = Theme.LabelActiveText
                else
                    TweenService:Create(togInner, TweenInfo.new(0.1), {Position = UDim2.new(0, 1, 0.5, -5), BackgroundColor3 = Theme.ToggleOff}):Play()
                    lbl.TextColor3 = Theme.LabelText
                end
            end
            updateVisual()
        ...
        function sec:Slider(sName, min, max, default, suffix, callback)
        ...
        function sec:Button(bName, callback)
            local btn = Instance.new("TextButton")
            btn.Parent = body
            ...
            btn.Text = bName:upper()
            btn.TextColor3 = Theme.ButtonText
            btn.TextSize = 10
            btn.MouseEnter:Connect(function()
                btn.TextColor3 = Theme.ButtonHoverText
            ...
            table.insert(themeRegistry.buttons, {Instance = btn})
        end

        function sec:Dropdown(dName, options, callback)
            local wrap = Instance.new("Frame")
            local lbl = Instance.new("TextLabel")
            ...
            box.Parent = wrap
            box.BackgroundColor3 = Theme.ButtonBG
            box.BorderColor3 = Theme.ButtonBorder
            ...
            -- Возвращаем функцию обновления опций (нужно для конфигов)
            local ctrl = {}
            function ctrl:SetOptions(newOptions)
                -- очищаем старые
                for _, child in ipairs(optHolder:GetChildren()) do
                    if child:IsA("TextButton") then child:Destroy() end
                end
                optHolder.Size = UDim2.new(0.5, 0, 0, #newOptions * 18)
                for _, opt in ipairs(newOptions) do
                    local ob2 = Instance.new("TextButton")
                    ob2.Parent = optHolder
                    ob2.BackgroundTransparency = 1
                    ob2.BorderSizePixel = 0
                    ob2.Size = UDim2.new(1, 0, 0, 18)
                    ob2.Font = Enum.Font.Code
                    ob2.Text = opt
                    ob2.TextColor3 = Color3.fromRGB(120, 120, 120)
                    ob2.TextSize = 10
                    ob2.ZIndex = 10
                    ob2.MouseEnter:Connect(function() ob2.TextColor3 = Color3.fromRGB(220,220,220) end)
                    ob2.MouseLeave:Connect(function() ob2.TextColor3 = Color3.fromRGB(120,120,120) end)
                ...
                if #newOptions > 0 then
                    boxLbl.Text = newOptions[1]
                else
                    boxLbl.Text = ""
                end
            end
            function ctrl:GetSelected()
                return boxLbl.Text
            end
            return ctrl
        end

        function sec:Textbox(tName, placeholder, callback)
            local wrap = Instance.new("Frame")
            local lbl = Instance.new("TextLabel")
            ...
            -- Возвращаем контрол для чтения/записи
            local ctrl = {}
            function ctrl:GetText() return box.Text end
            function ctrl:SetText(t) box.Text = t end
            return ctrl
        end

        function sec:Keybind(kName, default, callback)
            local row = Instance.new("Frame")
            local lbl = Instance.new("TextLabel")
            ...
            kb.Parent = row
            kb.BackgroundColor3 = Theme.ButtonBG
            kb.BorderColor3 = Theme.ButtonBorder
        ...
        function sec:ColorPicker(cpName, default, callback)
            local row = Instance.new("Frame")
            local lbl = Instance.new("TextLabel")
        ...
        -- Метки для обновления темы секции
...
-- KEYBINDS (только Toggle Menu, aimbot key убран)
bindSec:Keybind("Toggle Menu", Enum.KeyCode.RightShift, function(key)
    menuBind = key
end)

-- THEME — настройка цветов каждого элемента
themeSec:ColorPicker("Accent Color", Color3.fromRGB(255,255,255), function(color)
    Theme.Accent = color
    NeonStroke.Color = color
    WatermarkStroke.Color = color
end)

themeSec:ColorPicker("Button BG", Color3.fromRGB(14,14,14), function(color)
    Theme.ButtonBG = color
end)

themeSec:ColorPicker("Button Border", Color3.fromRGB(35,35,35), function(color)
    Theme.ButtonBorder = color
end)

themeSec:ColorPicker("Button Text", Color3.fromRGB(120,120,120), function(color)
    Theme.ButtonText = color
    for _, b in ipairs(themeRegistry.buttons) do
        if b.Instance and b.Instance.Parent then
            b.Instance.TextColor3 = color
        end
    end
end)

themeSec:ColorPicker("Label Text", Color3.fromRGB(140,140,140), function(color)
    Theme.LabelText = color
end)

themeSec:ColorPicker("Window BG", Color3.fromRGB(8,8,8), function(color)
    Theme.WindowBG = color
    MainFrame.BackgroundColor3 = color
    Watermark.BackgroundColor3 = color
end)

themeSec:ColorPicker("Section BG", Color3.fromRGB(12,12,12), function(color)
    Theme.SectionBG = color
end)
...
-- ==============================
-- CONFIGS (рабочая система)
-- ==============================
local FOLDER = "ScriptHub"
if not isfolder(FOLDER) then makefolder(FOLDER) end

local currentConfigName = ""
local cfgDropCtrl = nil  -- будет присвоен после создания дропдауна
    ...
    if isfolder(FOLDER) then
        for _, f in ipairs(listfiles(FOLDER)) do
            ...
            if fname:sub(-4) == ".txt" then
                table.insert(list, fname:sub(1, -5))  -- убираем .txt
            end
        end
    end
    return list
...
local function refreshDropdown()
    if cfgDropCtrl then
        ...
        cfgDropCtrl:SetOptions(list)
    end
end

-- Сохранение конфига — сохраняем Theme как простой текст key=r,g,b
local function saveConfig(name)
    ...
    data = data .. "Accent=" .. colorToStr(Theme.Accent) .. "\n"
    data = data .. "ButtonBG=" .. colorToStr(Theme.ButtonBG) .. "\n"
    data = data .. "ButtonBorder=" .. colorToStr(Theme.ButtonBorder) .. "\n"
    data = data .. "ButtonText=" .. colorToStr(Theme.ButtonText) .. "\n"
    data = data .. "LabelText=" .. colorToStr(Theme.LabelText) .. "\n"
    data = data .. "WindowBG=" .. colorToStr(Theme.WindowBG) .. "\n"
    data = data .. "SectionBG=" .. colorToStr(Theme.SectionBG) .. "\n"
    writefile(path, data)
end
...
local function loadConfig(name)
    ...
    for line in raw:gmatch("[^\n]+") do
        ...
        if key and val then
            if key == "Accent" then
                Theme.Accent = strToColor(val)
                NeonStroke.Color = Theme.Accent
                WatermarkStroke.Color = Theme.Accent
            elseif key == "ButtonBG" then
                Theme.ButtonBG = strToColor(val)
            elseif key == "ButtonBorder" then
                Theme.ButtonBorder = strToColor(val)
            elseif key == "ButtonText" then
                Theme.ButtonText = strToColor(val)
            elseif key == "LabelText" then
                Theme.LabelText = strToColor(val)
            elseif key == "WindowBG" then
                Theme.WindowBG = strToColor(val)
                MainFrame.BackgroundColor3 = Theme.WindowBG
                Watermark.BackgroundColor3 = Theme.WindowBG
            elseif key == "SectionBG" then
                Theme.SectionBG = strToColor(val)
            end
        end
    end
end
...
-- UI конфигов
local cfgNameCtrl = cfgSec:Textbox("Config Name", "myconfig", function(v)
    currentConfigName = v
end)

-- Dropdown выбора конфига
local initList = getConfigList()
if #initList == 0 then initList = {"(empty)"} end
cfgDropCtrl = cfgSec:Dropdown("Select Config", initList, function(v)
    if v ~= "(empty)" then
        currentConfigName = v
        if cfgNameCtrl then cfgNameCtrl:SetText(v) end
    end
end)
...
cfgSec:Button("Delete Config", function()
    ...
    deleteConfig(name)
    currentConfigName = ""
    if cfgNameCtrl then cfgNameCtrl:SetText("") end
    refreshDropdown()
end)
